...
func move():
	var decimalTimescale = fmod(timeScale, 1)
	if decimalTimescale == 0:
		for x in range(int(timeScale)):
		...
		for x in range(floor(timeScale)):
			for objectNode in groupNodes:
				if objectNode != self:
					var unitVector = relativePosition(objectNode).normalized()
					Velocity += unitVector * acceleration(gravitation(objectNode))/60.0
		...
		for objectNode in groupNodes:
				if objectNode != self:
					var unitVector = relativePosition(objectNode).normalized()
					Velocity += unitVector * acceleration(gravitation(objectNode))*decimalTimescale/60.0
		internalPosition += Velocity*PIXELSPERMETER*decimalTimescale/60.0
	global_position = internalPosition

func _physics_process(delta):
	if (Radius * 64) / get_parent().get_node("Camera2D").zoom.x < 5:
		$Distance.show()
		$Distance/Label.text = Name
		$Distance.rect_scale = get_parent().get_node("Camera2D").zoom / (Radius * 4)
	else:
		$Distance.hide()
	...
	$Particles2D.modulate = Color(color.x, color.y, color.z)
